# Generated by Powerlevel10k configuration wizard on 2023-08-12 at 00:58 CST.
# Based on romkatv/powerlevel10k/config/p10k-lean-8colors.zsh.
# Wizard options: powerline, unicode, lean_8colors, 2 lines, disconnected, no frame,
# sparse, fluent, instant_prompt=verbose.
...
() {
  ...
  ################################[ prompt_char: prompt symbol ]################################
  # Cyan prompt symbol if the last command succeeded.
  ...
  typeset -g POWERLEVEL9K_LOCK_ICON="${red_b}∅"
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      res+="${cyan}${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${magenta_b}${branch//\%/%%}"
    fi
    ...
    # in this case.
    (( VCS_STATUS_HAS_STAGED || VCS_STATUS_HAS_UNSTAGED || VCS_STATUS_HAS_CONFLICTED )) && res+="${red_b}*"
    # (( VCS_STATUS_HAS_UNTRACKED )) && res+="${red_b}?"
    ...
    if [[ -n $VCS_STATUS_TAG
      ...
      res+="${cyan}#${magenta_b}${tag//\%/%%}"
    fi

    # Display the current Git commit if there is no branch and no tag.
    # Tip: To always display the current Git commit, delete the next line.
    [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&  # <-- this line
      res+="${cyan}@${magenta_b}${VCS_STATUS_COMMIT[1,8]}"

    # Show tracking branch name if it differs from local branch.
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${cyan}:${magenta_b}${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"
    fi
    ...
    if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
      ...
      # (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${cyan}⇣${dark_b}${VCS_STATUS_COMMITS_BEHIND}"
      (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${cyan}⇣"
      # ⇡42 if ahead of the remote; no leading space if also behind the remote: ⇣42⇡42.
      (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
      # (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${cyan}⇡${dark_b}${VCS_STATUS_COMMITS_AHEAD}"
      (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${cyan}⇡"
    fi
    ...
    [[ -n $VCS_STATUS_ACTION     ]] && res+=" ${red_b}${VCS_STATUS_ACTION}"
  ...
  # Custom prefix.
  typeset -g POWERLEVEL9K_COMMAND_EXECUTION_TIME_PREFIX="${dark_b}took "
  ...
  # Custom prefix.
  typeset -g POWERLEVEL9K_CONTEXT_PREFIX="${dark_b}with "
  ...
  ##########[ nodenv: node.js version from nodenv (https://github.com/nodenv/nodenv) ]##########
  # Nodenv color.
  typeset -g POWERLEVEL9K_NODENV_FOREGROUND=8
