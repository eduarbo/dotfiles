...
function Label:show(duration)
    ...
    self.bgObj =
        drawing.rectangle(bgDisplayFrame):setStroke(false):setFill(true):setFillColor(fillColor):setRoundedRectRadii(
        5,
        5
    ):show(0.15)
    self.textObj = drawing.text(textDisplayFrame, self.message):setTextStyle(textStyle):show(0.15)
